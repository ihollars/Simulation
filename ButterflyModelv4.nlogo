globals [ ;q ;;q moved to interface via slider
...
to setup
  ...
  ; If interface switch "artificial?" is on, we use an artificial elevation;
  ; otherwise we use elevation data from a file
  ifelse artificial?
  [
    ; set elevation artificially
    ask patches
    [
  ...
  [
    ; Read and set elevation data from a file
  ...
  ; compute min and max elevation and color patches scaled with elevation
...
to go ; This is the master schedule
  ...
  tick
  if ticks >= 1000 [
    ; Next line, if not commented out, will save plot data to an external file
    ;  export-plot "Corridor width vs time" (word "corridor-output-for-q-" q ".csv")
    output-print word "Number of patches with Butterflies: " clumping-patch
    output-print word "Butterflies in radius averaged over all butterflies: "clumping-radius
    stop
  ]

end

;this code plots the corridor-width and the two clumping metrics over time
; the code to report these variables is found at at the bottom
to do-plotting
  set-current-plot "Main Plot"
  set-current-plot-pen "corridor-width"
  plot corridor-width
  ;set-current-plot-pen "clumping-patches"
  ;plot clumping-patch
  ;set-current-plot-pen "clumping-radius"
  ;plot clumping-radius

  ;set-current-plot "clumping-radius vs Time"
  ;set-current-plot-pen "clumpRad"
  ;plot clumping-radius
end
...
to move  ; The butterfly move procedure, in turtle context
  ...
  set used? true ; butterfly marks this patch as used

end ; of move procedure

to-report corridor-width
  ; corridor width is defined as the number of patches visited by butterflies divided by the mean distance
  ; of a butterfly from its start to its endpoint
  let num-patches-used count patches with [used?]
  let mean-dist-travelled mean [distance start-patch] of turtles
  report num-patches-used / mean-dist-travelled
end

to-report clumping-patch
  ; how many patches have butterflies on them?
  ; count the patches where the count of the turtles on the patch > 0
  let patch-count count patches with [count turtles-here > 0 ]
  report patch-count
end

to-report clumping-radius
  ; report the average number of butterflies in a fixed radius
  ; that fixed radius is defined by the slider radius-for-clumping-metric
  let cr mean [count turtles in-radius radius-for-clumping-metric ] of turtles
  report cr
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"corridor-width" 1.0 0 -7500403 true "" ""
